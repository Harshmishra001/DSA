// Problem Statement: Given two arrays representing children’s green factor and cookie sizes, the goal is to maximise the 
// number of content children.

// Each child i has a greed factor of g[i], which is the minimum size of a cookie that will make the child
//  content. Each cookie j has a size of s[j]. If s[j] >= g[j], we can assign cookie j to child i, making the
//   child content. Each child can only receive one cookie.
...
// Output: 1
// Explanation:  We have 3 children and 2 cookies. The greed factors of the 3 children are 1, 2, 3. The sizes 
// of the 2 cookies are both 1. We can only make the child with greed factor 1 content. Hence, the output is 1.

// **Time Complexity:** O(N logN + M logM + M) ≈ O(N logN + M logM)  
